...
5. 把产品需求文档 / 简报 / 头脑风暴中已确认的用户故事、候选功能和验收种子转成 `REQ-*`、`AC-*`、NFR、非目标、明确延后或待澄清项。
6. 需求正文使用系统行为语言，不复制产品需求文档原文，不写实现方案；优先使用 EARS 或 RFC 2119 层级表达。
...
9. 对中高复杂度需求执行一致性检查：用户目标、最小可行版本、角色权限、数据口径、验收标准和非目标不得互相冲突；冲突必须回到用户确认。
