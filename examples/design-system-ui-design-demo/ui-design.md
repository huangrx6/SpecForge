...
| 方案 | 适用性 | 结论 |
| --- | --- | --- |
| Fixed sidebar | 模块多、值班长高频切换四个工作区，桌面端需要稳定定位 | 推荐桌面采用 |
| Top nav | 横向空间更大，但四个模块加值班状态和搜索会拥挤 | 不作为桌面默认 |
| Hybrid top + sidebar | 适合多组织 / 多城市切换，本 demo 暂无复杂组织层级 | 后续扩展 |
| Command nav | 专家高频跳转可作为辅助，不替代主导航 | 作为后续快捷入口 |
| Mobile bottom nav | 移动端保留 3-4 个高频入口 | 移动端采用 |

桌面主 sidebar 和右侧 AI 决策栏固定，主内容独立滚动，避免长页面滚动后丢失定位和决策上下文。
