...
| Profile | 框定深度 | 输出重点 |
|---|---|---|
| `light` | 只框定目标、约束、必须确认项 | 让用户能快速选方向 |
| `deep` | 完整框定目标、受众、非目标、成功标准、可变空间 | 支撑后续发散、类比、场景和矩阵 |
| `research-heavy` | 重点识别事实问题和本地事实缺口 | 把查证问题交给 `research-source` |
...
- 不能在问题未框定前进入方案推荐。
- 输出必须能被 `divergent-thinking` 直接使用。
