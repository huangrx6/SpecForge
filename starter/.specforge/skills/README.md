...
- 每个入口技能是一个可维护的技能包：`skills/<sf-*>/SKILL.md` 是入口，`constraints/` 放该技能会读取的阶段约束，`scripts/commands.json` 放该技能关联的脚本命令。
- `core/workflows/stages/` 仅保留兼容镜像和底层发行资产；维护入口优先看对应的 `skills/<sf-*>/constraints/`。
