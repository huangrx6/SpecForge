...
if (!title) {
  console.error('Usage: node .specforge/execution/tools/create-change.mjs "Change title"');
  process.exit(1);
}
...
function copyTemplate(templateName, destination, replacements = {}) {
  let content = readFileSync(join(root, layout.templates, templateName), "utf8");
...
console.log(base);
console.log(`Next: node ${layout.tools}/create-artifact.mjs requirements`);
