...
export function listChanges(kind) {
  const dir = `.specforge/changes/${kind}`;
...
export function resolveChange(options = {}) {
  ...
  if (change) {
    const activeBase = `.specforge/changes/active/${change}`;
    if (exists(`${activeBase}/change.yaml`)) return { name: change, base: activeBase, lifecycle: "active" };

    const archiveBase = `.specforge/changes/archive/${change}`;
  ...
  if (active.length === 1) {
    const name = active[0];
    return { name, base: `.specforge/changes/active/${name}`, lifecycle: "active" };
  }
  if (active.length > 1) {
  ...
  if (!activeOnly && defaultToLatestArchive) {
    ...
    if (archived.length > 0) {
      ...
      return { name, base: `.specforge/changes/archive/${name}`, lifecycle: "archive" };
    }
  }
...
export function renderOutput(output) {
  ...
  return readText(`.specforge/templates/${templateName}`);
}
