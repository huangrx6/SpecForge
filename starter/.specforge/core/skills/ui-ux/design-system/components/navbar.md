...
如果需求没有明确导航模式，输出必须先给候选：

| Candidate | 适合点 | 放弃风险 |
|---|---|---|
| fixed sidebar | 模块多、工作台高频切换、需要稳定定位 | 宽度不足、移动端折叠成本 |
| top nav | 模块少、层级浅、内容需要横向空间 | 二级导航容易拥挤 |
| hybrid | 全局切换和模块内导航都重要 | 层级复杂，视觉负担高 |
| tabs / command nav | 同对象多视图或专家跳转 | 不可替代主导航 |

生成原型前必须记录推荐项和人工确认来源。没有确认时，只能作为可逆默认或回到 brainstorm。
...
- 主导航、右侧辅助栏和主内容共用一个滚动容器，长页面中丢失上下文
- 移动端菜单覆盖内容但没有返回路径
- 图标风格混乱，品牌区过度装饰
- 权限项忽隐忽现导致布局跳动
