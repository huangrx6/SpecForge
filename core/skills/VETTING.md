...
|---|---:|---:|---|---|
| `requirements` | SpecForge local-authored | 6 | 可能把确认边界规则当成用户已确认范围，或把产品 / 设计 / 技术方案误写成需求 | 只作为 requirements 行为契约、来源转译、REQ / AC / NFR、覆盖 pattern、质量修复和下游 handoff 主能力包 |
| `user-stories` | `phuryn/pm-skills` | 0 | 可能带入 Sprint / backlog 表达 | 只保留 3C、INVEST 和验收标准补充方法 |
| `design-system` | SpecForge local-authored | 58 | 可能把设计规则当成已确认 UI 方向，或把 Brand Surface 审美误套到 Product UI | 只作为设计模式路由、设计语言、专业色阶、机器可读契约、组件契约、动效、页面模式和视觉 QA 参考，必须归一化到 UI artifact 或组件契约 |
...
| `code-intelligence` | SpecForge local-authored | 11 | 可能把 provider 输出当成当前事实、全仓扫描替代 bounded context，或把 affected tests 当成已验证 | 只作为跨阶段代码事实能力包，必须把 provider / fallback 输出归一为 graph_facts、impact report 或 Wiki 回写候选，再由对应阶段 gate 消费 |
...
| `test-engineering` | SpecForge local-authored | 10 | 可能把测试计划替代真实验证，或错误处理登录态 / 证据 | 只作为测试工程能力包，必须把 TC / PW / runtime / auth / evidence 落到 verification artifact 并由 `sf-verify` 执行 gate |
...
`opportunity-solution-tree` 不再作为独立本地 skill 托管；已归并到 `product/references/external-ost-reference.md`，仅保留机会树、实验和优先级检查视角。
