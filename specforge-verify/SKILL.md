...
- `.specforge/rules/testing/README.md`：验证深度匹配风险。
- `.specforge/rules/gates/README.md`：verification evidence。
- `.specforge/rules/boundaries/README.md`：验证下游影响。
- `.specforge/rules/security/README.md`：安全敏感变更需要额外检查。
- `.specforge/rules/delivery/README.md`：发布、配置和运行态验证。
...
`specforge-verify` 负责收集和记录足以支撑当前 change 验收标准的证据。默认优先级：
...
| 生产发布 | 不属于 | 交给 `specforge-close` 的 release / rollback 记录处理 |

## verify 与 gate 的分工

- `specforge-verify` 负责选择验证方式、运行或收集证据、写 `05-verification/report.md` 和 `05-verification/ci-result.md`。
