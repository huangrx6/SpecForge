...
按任务选择 agent，不按习惯启动 agent。完整上下文规则见 `.specforge/rules/context/README.md`。

## 选择顺序

1. 先运行 `node .specforge/tools/instructions.mjs` 判断当前 ready artifact。
2. 根据 artifact 和风险选择一个主 agent。
3. 只有当问题明显跨专业边界时，才增加第二个 agent。
4. agent 输出不能替代 gate；gate 仍必须由 `.specforge/tools/gate.mjs` 写入。
