...
4. 没有现成导航规范时，单独给出 2-3 个导航候选（fixed sidebar / top nav / hybrid / tabs / command nav / mobile bottom nav / 无常驻导航），说明推荐项、放弃项和滚动区域；导航模式不和美学方向混在一起问。
5. 用户确认美学方向、业务翻译和导航模式后，立即写 `[UI DECISION CONFIRMED]` 或 `UI Direction Status: confirmed`，并记录确认来源。
6. 每轮只问会改变 UI 的关键问题或一小组强相关问题；问题数量不设硬上限，但要分轮收敛，并解释为什么这些问题会改变 UI design。
...
| Navigation mode | 多模块后台、轻量产品、移动端、专家工作台 | fixed sidebar / top nav / hybrid / tabs / command nav / mobile bottom nav 选型和滚动区域 |
...
- 导航模式：是否给出并记录候选选型；主导航是否固定、是否侧边、顶部或混合；内容滚动不能带走桌面主导航，除非有明确理由和用户确认。
- 滚动区域：全局导航、页面头部、主内容、右侧辅助栏、表格和局部列表的滚动边界是否清楚。
