...
2. **构建"页面×操作×角色"验证矩阵**（适用于包含 UI 的变更）：
   ...
   - 枚举涉及的角色（如：普通用户、管理员、未登录）。
   - 每个单元格必须有实际测试结果（通过 ✓ / 失败 ✗ / 跳过带理由）。
3. 根据风险选择测试层级：单元、集成、E2E、手工、静态检查、构建或 CI。
4. E2E 测试必须覆盖完整业务闭环，不能只测 happy path。
   - 典型的完整闭环示例：创建 → 提交审批 → 审批通过 → 执行 → 查看结果 → 下载。
   - 每个环节的异常态（如执行失败、文件下载 403）也必须有对应验证。
5. 实际运行可用命令，记录命令、结果、时间和关键输出摘要。
6. 无法运行时说明原因、替代证据和残余风险。
7. 有 CI 时记录链接、状态和失败摘要。
