...
function validateSkillSnapshot(skill, baseDir) {
  ...
  for (const file of skill.source?.files ?? []) {
    ...
    if (!exists(supportPath)) errors.push(`${skill.id}: missing support file ${relative(supportPath)}`);
  }
...
function validateNoUnregisteredSkillDirs(registryIds) {
  ...
  for (const entry of readdirSync(skillsRoot, { withFileTypes: true })) {
    ...
    if ((exists(skillPath) || exists(sourceJsonPath)) && !registryIds.has(entry.name)) {
...
function validateStarterMirror(skill, checkedFiles) {
  if (layout.kind !== "source" || !exists(starterSkillsRoot)) return;
  ...
  for (const file of checkedFiles) {
    ...
    if (exists(sourcePath) && read(sourcePath) !== read(starterPath)) {
