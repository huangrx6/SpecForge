...
4. 按“Profile Section 矩阵”决定 section 是否出现；不要同时参考旧骨架和 profile 表。
...
这是唯一的输出骨架来源。`必填` 必须出现；`按需` 只在影响决策或交接时出现；`N/A 可` 可以省略，也可以写 N/A 理由；`--` 默认不输出。
...
Section 顺序按表格顺序输出；不要再套用额外“默认完整模板”。`skip` profile 只解释为什么跳过 brainstorm、下一步路由和一个最高优先级问题。
