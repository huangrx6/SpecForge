...
本子模块是 `sf-design` 的内部参考，**只在本次 change 涉及数据库 Schema、索引、迁移或数据流变更时读取**。
...
涉及持久化时，先读取 `.specforge/policy/tech-profiles/README.md` 的数据库选择矩阵，再按实际候选读取：
...
如果使用 MongoDB、Redis、对象存储、搜索引擎或其他存储，但当前没有对应 profile，design 必须写入 `Profile Deviations`：为什么不适用现有 profile、风险是什么、如何验证。
