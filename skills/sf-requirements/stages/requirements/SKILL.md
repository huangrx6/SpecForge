...
本技能把 intake / PRD / brainstorm / research 转成可测试、可追踪、可交给设计和实现的行为契约。需求只描述用户、操作者、系统外部可观察行为，不提前写实现方案。
...
14. 对中高复杂度需求执行一致性检查：用户目标、MVP、角色权限、数据口径、验收标准、非目标和影响面不得互相冲突；冲突必须请求人工确认。
