...
本子模块是 `sf-tech-design` 的内部参考，**只在本次 work item 涉及 API、SDK、事件或跨系统契约变更时读取**。同时读取 `.specforge/core/standards/engineering.md`。
...
- 每个端点必须说明 consumer、owner、鉴权边界、兼容性策略和 contract test 方式。
- 对外或跨团队 API 必须说明日志字段、trace id、错误码可观测性和限流 / 重试语义。
...
- 变更必须写清发布顺序：先兼容服务端、再消费者迁移、最后移除旧契约；不能一次性破坏消费者。
- 需要对端确认的契约必须写阻断项、对接人 / 团队、验收证据和回退方案。
...
- Consumer / owner / contract test / observability / deprecation path。
...
- 对外 API 缺少错误码、兼容策略、consumer 验证或废弃路径。
