...
| `.specforge/schemas/<workflow>.json` | 声明 artifact、依赖、输出、门禁、apply 条件和 archive 条件。 |
| `.specforge/changes/active/<change-id>/change.yaml` | 记录单个 change 的 workflow、当前 stage、status、门禁状态和证据路径。 |
...
```text
.specforge/changes/active/<change-id>/
├── change.yaml
├── 00-intake/
│   ├── original-request.md
│   └── brief.md
...
权威 artifact 列表和依赖关系在 `.specforge/schemas/standard.json` 中声明；上面的目录树只是便于人阅读的视图。
...
| 技能 | 读取 | 写入 |
|---|---|---|
| `specforge-intake` | 用户请求、knowledge、registry | `change.yaml`、`00-intake/original-request.md`、`00-intake/brief.md` |
| `specforge-spec` | intake、rules、knowledge | `01-spec/requirements.md`、`01-spec/design.md`、`01-spec/tasks.md`、`02-spec-review/spec-review-v1.md` |
| `specforge-review` | spec 或 implementation artifacts | `02-spec-review/spec-review-v1.md` 或 `04-code-review/code-review-v1.md`，然后更新门禁 |
| `specforge-implement` | 已批准的 spec 和 tasks | 业务代码、`03-implementation/*`、task 勾选状态 |
| `specforge-verify` | design、tasks、implementation、review | `05-verification/report.md`、`05-verification/ci-result.md`，然后更新 verification 门禁 |
| `specforge-close` | verification、knowledge、closure rules | `06-closure/ssot-sync.md`、`06-closure/release.md`、`06-closure/rollback.md`，然后更新 ssot 门禁并 archive |
| `specforge-work` | artifact graph 和 instructions 输出 | 委托给阶段技能；不应发明绕过路径 |
...
```bash
node .specforge/tools/gate.mjs <gate> <status> --evidence <relative-path>
```
...
archive 命令会把 change 从 `.specforge/changes/active/` 移动到 `.specforge/changes/archive/`，并更新 `.specforge/registry.yaml`。
