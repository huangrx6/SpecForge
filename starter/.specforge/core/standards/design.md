...
导航模式是信息架构决策，不是视觉默认值。没有现成设计系统依据时，UI design 必须给出 2-3 个候选方案、推荐理由和风险，让用户确认后再进入原型。

| 模式 | 适用场景 | 主要风险 |
|---|---|---|
| **顶部导航栏** | 桌面端 3-7 个主要分区、模块层级浅 | 二级层级多时拥挤 |
| **固定侧边导航** | 深层级工具类产品、后台、控制台、高频模块切换 | 占用横向空间，移动端需要折叠 |
| **顶部 + 侧边混合** | 全局组织 / 产品切换 + 模块内导航 | 层级过重，用户容易迷路 |
| **底部导航栏** | 移动端 3-5 个主要操作（最佳可达性） | 不适合复杂管理系统 |
| **命令 / 搜索导航** | 专家用户、高频跳转、对象很多 | 新手发现性弱，不能作为唯一入口 |
| **汉堡菜单** | 仅用于次要导航，隐藏导航降低可发现性 | 高频入口被隐藏 |
| **标签页** | 同层级相关视图切换 | 不能替代主导航或流程步骤 |

导航决策必须同时说明：

- 主导航放在哪里：侧边、顶部、混合、底部、tabs、命令面板或无常驻导航。
- 主导航是否固定：桌面主导航采用 sidebar 或 topbar 时通常应 fixed / sticky，不随主内容滚动。
- 哪些区域滚动：全局导航、页面头部、主内容、右侧辅助栏和局部列表要明确滚动边界。
- 移动端如何收敛：bottom nav、drawer、top tabs、command search 或无常驻导航。
