...
| 阶段 | 负责 | 不负责 | 关键产物 |
|---|---|---|---|
| intake / brainstorm | 判断类型、拆分、研究、用户参与式取舍 | 不写完整规格或代码 | `00-intake/brief.md` |
...
### 4.1 Intake 是用户参与式 brainstorm

复杂需求不能直接被 AI 整理成“看似完整”的需求。现在 intake 必须先列：
...
| 门禁 | 通过条件 |
|---|---|
| Clarity Gate | 用户目标、MVP、范围或问题类型已确认 |
